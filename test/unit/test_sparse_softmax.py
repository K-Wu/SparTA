...
from sparta.specializer.kernels import (
    SparTASparseSoftmaxForwardKernel,
    SparTASparseSoftmaxBackwardKernel,
)
from sparta.specializer.funtional import (
    SparseBatchSoftmaxCtx,
...
def prepare_data(
    ...
    mask = block_mask(
        (H, W), block=granularity, sparsity=sparsity, device="cuda"
    )
    ...
    data["target_y"] = sparse_softmax_forward_reference(
        data["input_x"], mask, temperature
    )
...
def test_sparse_softmax_kernels(
    ...
    data, mask = prepare_data(
        batch, H, W, granularity, sparsity, requires_grad=True
    )
    ...
    backward_kernel.test(
        backward_inputs, num_warmups=0, num_iters=1, cuda=False
    )
...
def test_sparse_softmax_function(
    ...
    for port_name, ports in sparse_ctx.sparse_ports.items():
        for port in ports:
            port.set_mask(mask)
    sparse_ctx.build(
        {kernel_name: get_params() for kernel_name in kernel_names}
    )
    ...
    data["output_y"] = SparseBatchSoftmaxFunc.apply(
        sparse_ctx, data["input_x"]
    )
...
def test_sparse_softmax_operator(
    ...
    sparse_softmax.build(
        config={
            ...
            for kernel_name in sparse_softmax.get_kernel_placeholders(
                backward=True
            )
        },
    ...
    for random_seed in range(3):  # Test dynamic sparse
        ...
        data, mask = prepare_data(
            batch, H, W, granularity, sparsity, True, random_seed
        )
