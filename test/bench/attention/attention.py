...
from sparta.testing import (
    block_mask,
    profile,
    sparse_multi_head_attention_reference,
)
...
def profile_triton_attention(
    ...
    data, mask = prepare_data(
        Ns, Nt, E, granularity, sparsity, ndim=4, device=device
    )
    if mask.sum() == 0:
        return 0.0, 0.0

    layout = mask.reshape(
        Nt // block_size, block_size, Ns // block_size, block_size
    )
    layout = (
        layout.swapaxes(1, 2)
        .any(-1)
    ...
    sparse_softmax = triton.ops.blocksparse.softmax(
        layout=layout, block=block_size, device=device
    )
    scale = 1 / np.sqrt(E)
...
def profile_sparta_attention(
    ...
    data, mask = prepare_data(
        Ns, Nt, E, granularity, sparsity, ndim=3, device=device
    )
    if mask.sum() == 0:
        return 0.0, 0.0
    ...
    sparse_attention.build(
        config, sample_inputs=[data["query"], data["key"], data["value"]]
    )
...
def get_sparta_config(
    configs: pd.DataFrame, granularity: int, sparsity: float
):
    condition = (configs["granularity"] == granularity) & (
...
def profile_all(log_path: str, device: Any = "cuda"):
    ...
    for g in GRANULARITY_LIST:
        for s in SPARSITY_LIST:
            ...
            latency["dense"] = profile_dense_attention(
                Ns, Nt, E, (g, g), s, device
            )
            config = get_sparta_config(sparta_configs, g, s)
            latency["sparta"] = profile_sparta_attention(
                config, Ns, Nt, E, (g, g), s, device
            )
            for block in [16, 32, 64]:
                latency[f"triton-{block}"] = profile_triton_attention(
                    block, Ns, Nt, E, (g, g), s, device
                )
            with open(log_path, "a") as f:
                for method, (lat_f, lat_b) in latency.items():
                    f.write(
                        f"{method},{Ns},{Nt},{E},{g},{s},{lat_f},{lat_b}\n"
                    )
            print(
                tabulate.tabulate(
                    tabular_data=[
