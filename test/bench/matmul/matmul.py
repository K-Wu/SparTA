...
def profile_matmul(
    ...
    def matmul_forward_backward(
        A: torch.Tensor, B: torch.Tensor, grad_C: torch.Tensor
    ):
        C = func(A, B)
        C.backward(grad_C)
...
def profile_triton_matmul(
    ...
    data, mask = prepare_data(
        M, K, N, granularity, sparsity, ndim=4, device=device
    )
    if mask.sum() == 0:
        return 0.0, 0.0
    ...
    data["grad_B"] = triton.testing.sparsify_tensor(
        data["grad_B"], layout, block_size
    )
...
def profile_sparta_matmul(
    ...
    data, mask = prepare_data(
        M, K, N, granularity, sparsity, ndim=3, device=device
    )
    if mask.sum() == 0:
        return 0.0, 0.0
...
def profile_all(log_path: str, device: Any = "cuda"):
    ...
    for g in GRANULARITY_LIST:
        for s in SPARSITY_LIST:
            ...
            latency["sparta"] = profile_sparta_matmul(
                config, M, K, N, (g, g), s, device
            )
            for block in [16, 32, 64]:
                latency[f"triton-{block}"] = profile_triton_matmul(
                    block, M, K, N, (g, g), s, device
                )
