...
from sparta.testing import (
    sparse_softmax_forward_reference,
    sparse_softmax_backward_reference,
)
...
class SparseSoftmaxForwardKernel(SparseSoftmaxKernel):
    ...
    def _set_ports(self):
        self.ports["x"] = PortConfig(
            name="x", is_input=True, is_sparse=True, BCSR=True
        )
        self.ports["y"] = PortConfig(
            name="y", is_input=False, is_sparse=True, BCSR=True
        )
...
class SparseSoftmaxBackwardKernel(SparseSoftmaxKernel):
    ...
    def _set_ports(self):
        self.ports["grad_y"] = PortConfig(
            name="grad_y", is_input=True, is_sparse=True, BCSR=True
        )
        self.ports["y"] = PortConfig(
            name="y", is_input=True, is_sparse=True, BCSR=True
        )
        self.ports["grad_x"] = PortConfig(
            name="grad_x", is_input=False, is_sparse=True, BCSR=True
        )
    ...
    def update_func(self):
        ...
        def softmax_backward_func(
            grad_y: torch.Tensor, y: torch.Tensor, T: np.int32
        ):
...
class SparTASoftmaxKernel(SparseSoftmaxKernel):
    ...
    def get_kernel_code(self):
        template_file = (
            f"{self.__algo__}_sparse_softmax_{self.__direction__}.cuh.j2"
        )
        kernel_template = res.read_text(templates, template_file)
        return jinja2.Template(kernel_template).render(self.get_parameters())


class SparTASparseSoftmaxForwardKernel(
    SparseSoftmaxForwardKernel, SparTASoftmaxKernel
):
    pass


class SparTASparseSoftmaxBackwardKernel(
    SparseSoftmaxBackwardKernel, SparTASoftmaxKernel
):
    pass
